...
4. Responda sempre em Português do Brasil.`;
      ...
      res.json({ text: result.text.replace(/\*/g, '') });
    } catch (error) {
  ...
  app.post("/api/summary", async (req, res) => {
    try {
      ...
      res.json({ text: response.text?.replace(/\*/g, '').trim() || "Materiais de construção" });
    } catch (error) {
      console.error("Summary Error:", error);
      res.status(500).json({ error: "Erro ao processar resumo" });
    }
  });
